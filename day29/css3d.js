...
C = {
  ...
  camera: o => {
    ...
    if(o && (o.el || o.el === 0)) C.el = o.el;
    C.camX += ((Math.random() - .5)/1000);
    if(top.scene){
      scene.style.transformOrigin = `${C.camX}${C.unit} ${C.camY}${C.unit} ${C.camZ-C.el}${C.unit}`;
      scene.style.transform = `translateX(${-C.camX}${C.unit})translateY(${-C.camY}${C.unit})translateZ(${-C.camZ}${C.unit})rotateX(${C.camRX}deg)rotateY(${C.camRY}deg)rotateZ(${C.camRZ}deg)translateZ(${-C.el}${C.unit})`;
